...
// if($keys[0]===0)continue;
 ...
 $lmap = 'typedef CArray<'."$bname> C$bname"."Map;\n";
...
//    case 'in':
 ...
 // $lmap=str_replace ( "#", $xml->count(), $lmap );
...
public:
    ...
    bool LoadCommon(const char *file, std::array<KeyType,N> &keys,std::array<T,M> &t1);
    template<typename T,size_t N>
    const T* GetConfig(std::array<T,N> &vec,uint32_t id)
    {
        if(vec.size()>id&&id>0)
        {
            return &vec[id-1];
...
template<class T,size_t N,size_t M>
bool XmlConfig::LoadCommon(const char *file, std::array<KeyType,N> &keys,std::array<T,M> &t1)
{
